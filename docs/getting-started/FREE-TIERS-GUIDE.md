...
> **TL;DR**: OmniRoute registers 352 provider IDs, with **152 provider-catalog entries marked `hasFree`**. The stricter audited free-model catalog covers **34 recurring pool keys / 447 entries** (440 active + 7 discontinued). Connect several suitable providers for broader fallback capacity; every quota, approval rule, privacy policy, and paid-overage condition still applies.
...
| Audited free-model inventory                         | **34 recurring pool keys / 447 catalog entries** | 440 active + 7 discontinued; distinct from the 352-provider catalog                                                        |
