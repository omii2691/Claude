...
// Lyceum (#12474, pay-per-use OpenAI-compatible gateway) adds one gateways entry — 239.
...
test("APIKEY_PROVIDERS merges the 6 family files into 239 entries (no loss / no dup)", async () => {
  ...
  assert.ok(keys.includes("lyceum"), "Lyceum (#12474) must land in the merged APIKEY catalog");
